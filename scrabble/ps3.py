...
def get_word_score(word, n):
    ...
    if HAND_SIZE * len(word) - 3 * (n-len(word)) < 1:
        secondComponent = 1
    else:
        secondComponent = HAND_SIZE * len(word) - 3 * (n-len(word))
    return s * secondComponent  
...
def is_valid_word(word, hand, word_list):
    ...
    word = word.lower()
    if '*' not in word:
        if word not in [lowerWord.lower() for lowerWord in word_list]:
            return False
        else:
            for l in word:
                if l in hand:
        ...
        index = word.index('*')
        for v in VOWELS:
            w = word[0:index] + v + word[index+1:]
            if w in [lowerWord.lower() for lowerWord in word_list]:
                for l in w:
                    if l in hand:
                         # What if word uses * as a vowel for a word that already exists and exceeds the occ
                        occ = word.count(l)
                        if occ > hand[l]:
                            if l.lower()==v:
                                extra_vowel += 1
